...
export class SyncManager implements ISyncManager {
  ...
  async processPush(payload: SyncPayload) {
    ...
    for (const type of objectTypes) {
      ...
      let newObjects = objects.filter(
        (obj) => obj?.metadata?.syncOperation == SyncOperationEnum.add
      );
      let updatedObjects = objects.filter(
        (obj) => obj?.metadata?.syncOperation == SyncOperationEnum.update
      );
      let deletedObjects = objects.filter(
        (obj) => obj?.metadata?.syncOperation == SyncOperationEnum.delete
      );
      let objectRepository = FastSync.getInstance().getObjectRepository(type);
      if (newObjects.length > 0)
        await objectRepository.addMany(
          newObjects,
          payload.getTypeMetadata(type)
        );
      if (updatedObjects.length > 0)
        await objectRepository.updateMany(
          updatedObjects,
          payload.getTypeMetadata(type)
        );
      if (deletedObjects.length > 0)
        await objectRepository.removeMany(
          deletedObjects,
          payload.getTypeMetadata(type)
        );
    }
  ...
  async processPull(metadata: SyncOperationMetadata): Promise<SyncPayload> {
    ...
    for (const type of requestedTypes) {
      ...
      let objects = await objectRepository.fetchMany(
        metadata.getTypeMetadata(type)
      );
      await syncPayload.pushObjects(
        type,
        objects,
        typeMetadata.computeSyncZone(fastSync.getSyncZoneConfiguration(type))
      );
    }
    return syncPayload;
  }
  ...
  private validateMetadataSyncZones(
    operatationMetadata: SyncOperationMetadata
  ) {
    let objectTypes = operatationMetadata.getSynckedTypes();
    for (const type of objectTypes) {
      let typeMetadata = operatationMetadata.getTypeMetadata(type);
      if (
        isEmptyString(typeMetadata.getSyncZone()) &&
        SyncZoneRestrictionEnum.restricted ==
          FastSync.getInstance().getSyncZoneConfiguration(type)
      ) {
        throw new Error(
          "type" +
            type +
            "is configured to be restricted, but no syncZoneAttribute was provided in the sync operatation metadata"
        );
      }
    }
  }
